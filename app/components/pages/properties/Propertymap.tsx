...
import { FaBath, FaBed, FaRegCalendar } from "react-icons/fa";
import { IoSquareOutline, IoVideocam } from "react-icons/io5";
...
// Kerala center (good default)
const center = {
    lat: 10.8505,
    lng: 76.2711,
};

export default function MapSection({ properties = [] }: any) {
    ...
    return (
        <LoadScript googleMapsApiKey="AIzaSyDCXLVnkQ1U6QfQtxw0-eEh0XOiiU6fFow">
            <GoogleMap mapContainerStyle={containerStyle} center={center} zoom={7} >
                    ...
                    {(clusterer) =>
                        properties.map((p: any) => (
                            <Marker
                                ...
                                // 🟡 PRICE LABEL MARKER
                                label={{
                                    text: p.price.replace(" Lakh", "L").replace(" Crore", "Cr"),
                                    color: "white",
                                    fontSize: "12px",
                                    fontWeight: "bold",
                                }}
                ...
                {selected && (
                    ...
                    >
                        <div className="w-52  h-180 p-5">
                            <img
                                src={selected.image}
                                className="rounded mb-2 w-full h-24 object-cover"
                            />
                            <h3 className="font-semibold text-sm">{selected.title}</h3>
                            <p className="text-xs">{selected.district}</p>
                            <div className="flex justify-between ">
                                <p className="text-orange-500 font-bold text-2xl" style={{ fontWeight: "600" }}>
                                    {selected.price}
                                </p>
                            ...
                            <div className="flex justify-between gap-2 pt-5">
                                <button onClick={() => setModalStatus(true)} type="button"  className="flex-1 cursor-pointer bg-gray-800 py-3 px-4 rounded-lg hover:bg-gray-700 transition text-white">
                                    Schedule Visit
                                </button>

                                <Link href={`/property/${selected.slug}`}>
                                    <button className="flex-1 bg-linear-to-r from-orange-500 to-yellow-500 py-3 px-4 rounded-lg text-black font-semibold">
                                        Details
                                    </button>
                                </Link>
                            </div>
                                                    ...
                                                    <div className="bg-black w-full max-w-md p-6 rounded-2xl text-white">
                                                        ...
                                                        <div className="text-center mb-6">
                                                            ...
                                                            <p className="text-gray-400 text-sm mt-1">
                                                                Experience Luxury {selected.overview}
                                                            </p>
                                                        </div>
